...
function SubscriptionPage() {
  ...
  const { form, onChange } = useForm({ name: "", age: "18", profession: "", reason: "" , country: ""});
  ...
  const applyToTrip = (tripId) => {
    ...
    axios.post(`https://us-central1-labenu-apis.cloudfunctions.net/labeX/leonardo-gomes/trips/${tripId}/apply`, body)
    .then(response =>{
      alert("Dados enviados para avaliação!")
    })
    .catch(err =>{
      console.log(err.message)
    })
  ...
  const changeSelectTrip = event => {
    setIdTrip(event.target.value)
  }

  return (
    <Subscribe>
      <Form onSubmit={applyToTrip(idTrip)}>
        <InputSub
          value={form.name}
          name= "name"
          placeholder= "Nome"
